...
function them_sp_gio_hang(id_nd, id_poduct) {
    if(id_nd=="") {
        ...
        url= '/product/add_cart/'+id_nd+'/'+id_poduct+'/'+so_luong;
...
function up_sl(id_sp, don_gia, list_cart, sl_max) {
    ...
    url= '/cart/up_sl/'+sl+'/'+id_sp+'/'+don_gia;
    xml.open("GET", url, "false");
    xml.send();
}
...
function mua_hang() {
    if(typeof sp_thanh_toan === 'undefined' || sp_thanh_toan.length == 0) {
        ...
        location.href= "/thanh_toan/view/"+sp_thanh_toan;
    }
}
...
function bo_loc() {
    ...
    url= '/bo_loc/'+sx+'/'+gia+'/'+ram+'/'+rom+'/'+tu_khoa;
    xml.open("GET", url, "false");
    xml.send();
}
